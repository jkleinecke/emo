...
impl Rom {
    pub fn new(raw: &[u8]) -> Result<Rom, String> 
    {
        ...
        let four_screen = raw[6].bit(3);
        let vertical_mirroring = raw[6].bit(0);
        ...
        let skip_trainer = raw[6].bit(2);

        let prg_rom_start = 16 + ternary!(skip_trainer, 512, 0);
    ...
    pub fn from_file(filepath: &str) -> Result<Rom, String> 
    {
        let bytes: Vec<u8> = std::fs::read(filepath).map_err(|e| e.to_string())?;
