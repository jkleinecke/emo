...
pub fn trace(cpu:&mut Cpu, memory:&mut dyn MemoryMapped) -> String {
    ...
    let tmp = match instr.opsize {
        ...
        3 => {
            ...
            let opaddress = Word::make(context.memory.read(op16+1),context.memory.read(op16));
            let address = fetch_operand_address(&mut context);
            let value = context.memory.read(address);
            
            match instr.addr_mode {
                AddressingMode::Indirect => format!("(${:04x}) = {:04x}", opaddress, address),
                AddressingMode::Absolute => format!("(${:04x}) = {:02x}", address, value),
    ...
    let asm_str = format!("{:04x}  {:8}  {: >4} {}", instr.ir_address, hex, instr.opcode, tmp).trim().to_string();
...
mod test {
    ...
    fn test_format_trace() {
        ...
        assert_eq!(
            "0064  A2 01     LDX #$01                        A:01 X:02 Y:03 P:34 SP:FD",
            t1
        );
        ...
        assert_eq!(
            "0066  CA        DEX                             A:01 X:01 Y:03 P:34 SP:FD",
            t2
        );
        ...
        assert_eq!(
            "0067  88        DEY                             A:01 X:00 Y:03 P:36 SP:FD",
            t3
        );
    }
    ...
    fn test_format_mem_access() {
        ...
        let t1 = trace(&mut cpu, &mut ram);
        assert_eq!(
            "0064  11 33     ORA ($33),Y = 0400 @ 0400 = AA  A:00 X:00 Y:00 P:34 SP:FD",
            t1
        );
    }
}