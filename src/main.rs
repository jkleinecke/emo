
mod common;
mod system;
mod bus;
mod operations;
mod cpu6502;
mod tests;

extern crate bitflags;

use sdl2::event::Event;
use sdl2::EventPump;
use sdl2::keyboard::Keycode;

use sdl2::pixels::PixelFormatEnum;
use rand::Rng;

use crate::system::Nes;
use crate::bus::Bus;

fn handle_user_input(bus: &mut Bus, event_pump: &mut EventPump)
{
    for event in event_pump.poll_iter()
    {
        match event
        {
            Event::Quit {..} | Event::KeyDown { keycode: Some(Keycode::Escape), .. } => {
                std::process::exit(0);
            },
            Event::KeyDown { keycode: Some(Keycode::W), .. } => {
                bus.store(0xffu16, 0x77);
            }
            Event::KeyDown { keycode: Some(Keycode::S), .. } => {
                bus.store(0xffu16, 0x73);
            }
            Event::KeyDown { keycode: Some(Keycode::A), .. } => {
                bus.store(0xffu16, 0x61);
            }
            Event::KeyDown { keycode: Some(Keycode::D), .. } => {
                bus.store(0xffu16, 0x64);
            }
            _ => {/* do nothing */}
        }
    }
}

fn main() {
    let mut nes = Nes::new();

    let game_code = vec![
        ,
    ];

    nes.cpu.bus.vram_dirty = true;
    nes.load_program(&game_code);

    let mut rng = rand::thread_rng();

    // init SDL2
    let sdl_context = sdl2::init().unwrap();
    let video_subsystem = sdl_context.video().unwrap();
    let window = video_subsystem
        .window("Snake game", (32.0 * 10.0) as u32, (32.0 * 10.0) as u32)
        .position_centered()
        .build().unwrap();

    let mut canvas = window.into_canvas().present_vsync().build().unwrap();
    let mut event_pump = sdl_context.event_pump().unwrap();
    canvas.set_scale(10.0, 10.0).unwrap();
    

    let creator = canvas.texture_creator();
    let mut texture = creator.create_texture_target(PixelFormatEnum::RGB24, 32, 32).unwrap();

    nes.cpu.bus.init_vram();
    loop
    {
        // could probably handle the input and video texture copies
        // directly as part of the bus, but this will work fine..
        handle_user_input(&mut nes.cpu.bus, &mut event_pump);

        //nes.cpu.bus.store(0x00FE, rng.gen_range(1,16) as u8);

        nes.step_instruction();

        if nes.cpu.bus.vram_dirty
        {
            println!("vram dirty!!");
            nes.cpu.bus.vram_dirty = false;
            texture.update(None, &nes.cpu.bus.vram, 32 * 3).unwrap();

            canvas.copy(&texture, None, None).unwrap();

            canvas.present();
        }

        // sleep to slow the game down a little bit
        ::std::thread::sleep(std::time::Duration::new(0, 70_000));
    }
}
